...
;(function(DocsAPI, window, document, undefined) {
    ...
    */
    ...
    //placeholder: this parameter could be object or string (for backward compatibility)
    DocsAPI.DocEditor = function(placeholder, config) {
        var placeholderId = typeof placeholder === "string"? placeholder : placeholder.id;
        ...
        _config.documentServerURL = config.documentServerURL;
        ...
        var _checkConfigParams = function() {
            if (_config.document) {
                ...
                if (!_config.document.key) {
                    ...
                    console.log("The \"document.key\" parameter for the config object must be string. Please correct it.");
                    return false;
                }
        ...
        var target = (typeof placeholder === "string") ? document.getElementById(placeholderId):placeholder;
        ...
        if (target && _checkConfigParams()) {
            ...
            console.log(`${new Date().toISOString()} [FATAL] [OnlyOffice] Parameters to initialize iframe are missing`);
        }
...
/*
    function getBasePath() { //this kill the performance on Chrome
...
*/
    ...
    function createIframe(config) {
        ...
        const DOCUMENT_SERVER_URL = config.documentServerURL || DEFAULT_DOCUMENT_SERVER ;
        const basePath = `${DOCUMENT_SERVER_URL}/web-apps/apps/`;

        let appPath = getAppPath(config);
        let appParameters = getAppParameters(config);

        iframe.src = basePath + appPath + appParameters;
        ...
        console.log(`%c[${new Date().toISOString()}] [iframe src]: ${iframe.src}`,"color:#ADE899;");
